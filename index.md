...
1. When executing the script, a series of questions with defaults answers are presented to the end-user. By just pressing **ENTER** assumes the defualt instance-id and AWS region.
...
1. The script will poll for the external IP address (either ephemeral or elastic) of the instance and inject the IP address into a PCoIP URI string to connect to the instance. The assumption is that PCoIP client is already install on the client and is atleast 20.01 or greater for URIs support.
